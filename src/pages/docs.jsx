  ...
  predict(model, videoElem) {
    if (this.state.classify) {
      tf.tidy(() => {
        ...
        tensor = tf.image.resizeBilinear(tensor, [model.input.shape[0], model.input.shape[1]]);
        const eTensor = tf.reshape(tensor, [1, model.input.shape[0], model.input.shape[1], 3]);
